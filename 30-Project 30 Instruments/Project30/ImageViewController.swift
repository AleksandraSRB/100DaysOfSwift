...
class ImageViewController: UIViewController {
    // Solution 3 - creating weak reference stops retain cycle
    ...
    override func viewDidLoad() {
        ...
        // Solution
        // Challange 1
        guard let path = Bundle.main.path(forResource: image, ofType: nil) else { fatalError("Could not find path") }
        guard let original = UIImage(contentsOfFile: path) else { fatalError("Could not load image from path") }
